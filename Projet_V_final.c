...
printf("  ---------------------\t\t\t\t         /    \\                                    -----------------\n");
printf("  | Ilya SILVA, BEN   |\t\t\t\t        /      \\                                   |  Mr.FERDOS    | \n");
printf("  | NUSCHKAH,JOHN     |\t\t\t\t       /        \\                                  -----------------\n");
printf("  --------------------\t\t\t\t      /          \\ \n");
printf("\t\t\t\t\t\t     /____________\\ \n");
        printf("\t\t\t\t\t\t      |GROUPE.11| \n");
    printf("\n\n");
    printf("\t\t\t\t\t---------------------------------------------------------\n");
    printf("\t\t\t\t\t|1|\t  Donner le nom de votre fichier d'ouvrages:        \n"); //les_ouvrages.txt
    printf("\t\t\t\t\t---------------------------------------------------------\n");
    printf("\n\n");
...
for (i=0;i<NOVA;i++){
    ...
    fflush(stdin);
	scanf("%s",&x.Titre);
	printf("\t\tLe livre a ete ajouter avec succe!");
...
PlisteOV SupprimerOV (PlisteOV *T){
	...
	while(P!=NULL ){
		if(strcmp(P->VAL1.Titre ,titre )!=0){
			Pred=P;
			printf("\t\tLe livre a ete supprimer avec succe!");
		P=P->Suivo;
		}else{
			break;
		}
...
PlisteET CreerFIFOET() {
 PlisteET  P=NULL,T ; PlisteET P1;
    ...
    printf("\t\t\t\t\t----------------------------------------------------------\n");
    printf("\t\t\t\t\t|2|\t  Donner le nom de votre fichier d'etudiant:        |\n");  //les_etudiants.txt
    printf("\t\t\t\t\t----------------------------------------------------------\n");
    printf("\n\n");

	//printf("	 ");
	gets(nomFich);
	fichier = fopen(nomFich, "r");//Elle tente ensuite d’ouvrir ce fichier en mode lecture ("r")
	/*if(fichier==NULL){
		do{ //
			printf("	//Nous n'avons pas trouver votre fichier, \n	//Si votre fichier ne trouve pas au meme dossier que votre programme\n	//Donner le chemin absolu: ");
			gets(nomFich);
     ...
     rewind(fichier);  //La boucle principale de la fonction lit chaque ligne du fichier à l’aide de fgets.
     	while (!feof(fichier)){
     		fgets(chaine, 100, fichier); i=0;
     		n=strlen(chaine);if(chaine[n-1]=='\n'){ chaine[n-1]='\0';}
     		char *token = strtok(chaine,tmp1); //Elle découpe la ligne en morceaux (tokens) en utilisant l’espace comme délimiteur grâce à la fonction strtok.
...
PlisteEP CreerFIFOEP() {
 PlisteEP  P=NULL,T ; PlisteEP P1;
    ...
    printf("\n\n");
    printf("\t\t\t\t\t-----------------------------------------------------------\n");
    printf("\t\t\t\t\t|3|\t  Donner le nom de votre fichier des Emprunts:       |\n");  //les_etudiants.txt
    printf("\t\t\t\t\t-----------------------------------------------------------\n");
    printf("\n\n");
...
fflush(stdin);
  ...
  printf("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~<< Merci d'avoir utiliser notre Bibiotheque >>~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
